...
(function (h, A) {
    ...
    var w = Object.prototype.toString, f = /(.*)!(.+)/, g = /\Wrequire\((['"]).+?\1\)/g, d = /\.(js|json)$/, j = /^\.+?\/.+/, r = /(^|\/)\.\//g, c = /[^/\/\.]+\/\.\.\//, n = /^([\w\-]+)\//, m = {
        require: 1,
        exports: 1,
        module: 1,
        host: 1,
        finish: 1
    }, e = {mods: {}}, o = {}, k = {}, a = {}, v = {}, y, p, q = {}, t = Array.prototype.forEach || function (D, E) {
            for (var C = 0, B = this.length; C < B; C++) {
                if (C in this) {
                    D.call(E, this[C], C, this)
                }
            }
        };
    ...
    A.define = function (i, G, F) {
        var E = typeof F === "string";
        if (!F) {
            if (G) {
                if (x(G)) {
                    F = A.filesuffix(A.realname(A.basename(i)))
                } else {
                    F = G;
                    G = null
                }
        ...
        i = i && A.realname(i);
        var C = i && e.mods[i];
        if (!e.debug && C && C.name && (E && C.loaded == 2 || C.exports)) {
            return
        }
        if (E && e.enable_ozma) {
            G = null
        ...
        var D = l(this) ? this : h;
        C = e.mods[i] = {name: i, url: C && C.url, host: D, deps: G || []};
        if (i === "") {
            y = C
        }
        if (typeof F !== "string") {
            ...
            var B = e.aliases;
            if (B) {
                F = F.replace(/\{(\w+)\}/g, function (I, H) {
                    return B[H] || ""
                })
            }
            C.url = F
        }
        if (C.block && !b(C.block)) {
            C.exports = F
        }
    };
    A.define.amd = {};
    A.require = function (I, D, E) {
        if (typeof I === "string") {
            if (!D) {
                I = A.realname(A.basename(I, p));
                return (e.mods[I] || {}).exports
            }
            I = [I]
        } else {
            if (!D) {
    ...
    A.exec = function (J) {
        ...
        while (K = J.pop()) {
            ...
            if (!K.block || !K.running && K.exports !== undefined) {
                continue
            }
            D = [];
            F = {};
            B = {id: K.name, filename: K.url, exports: F};
            N = K.deps.slice();
            N[K.block.hiddenDeps ? "unshift" : "push"]("require", "exports", "module");
            for (var H = 0, E = N.length; H < E; H++) {
                L = N[H];
                switch (L) {
                    case"require":
                        D.push(A.require);
                    ...
                    case"finish":
                        ...
                        break;
                    default:
                        D.push(((q[L] || []).pop() || e.mods[A.realname(L)] || {}).exports);
                        break
                }
            }
            if (!K.running) {
                p = K;
                P = K.block.apply(K.host, D) || null;
                p = false;
                F = B.exports;
                K.exports = P !== undefined ? P : F;
                for (var M in F) {
                    if (M) {
                        K.exports = F
                    }
    ...
    A.fetch = function (B, i) {
        var D = B.url, F = o[D];
        if (!F) {
            var E = B.name, G = k;
            if (B.deps && B.deps.length && G[E] !== 1) {
                G[E] = [B.deps.length, i];
                t.call(B.deps, function (H) {
                    var I = e.mods[A.realname(H)];
                    if (this[H] !== 1 && I.url && I.loaded !== 2) {
                        if (!this[H]) {
                            this[H] = []
                        }
                        this[H].push(B)
            ...
            F = o[D] = [[i, B]];
            var C = /^\w+:\/\//.test(D) ? D : (e.enable_ozma && e.distUrl || e.baseUrl || "") + (e.enableAutoSuffix ? A.namesuffix(D) : D);
            A.load.call(B.host || h, C, function () {
                t.call(F, function (H) {
                    H[0].call(H[1])
                });
                o[D] = 1;
                if (a[E] && a[E] !== 1) {
                    t.call(a[E], function (I) {
                        var H = this[I.name];
                        if (--H[0] <= 0) {
                            this[I.name] = 1;
                            A.fetch(I, H[1])
    ...
    A.load = function (i, E) {
        var D = l(this) ? this.document : h.document, C = D.createElement("script");
        ...
        C.onload = C.onreadystatechange = function (G, F) {
            if (F || !C.readyState || /loaded|complete/.test(C.readyState)) {
    ...
    A.scan = function (E, L, I) {
        ...
        if (E[1]) {
            ...
            var J = A.realname(B);
            if (!e.mods[J] && !m[J]) {
                var C = A.realname(A.basename(B, L));
                if (J !== C) {
                    e.mods[L.url + ":" + J] = C;
                    J = C
                }
                if (!e.mods[C]) {
                    A.define(C, A.filesuffix(C))
                }
            }
            E = L = e.mods[J];
            if (E) {
                if (G === "new") {
                    E = {is_reset: true, deps: E.deps, name: J, newname: G + "!" + J, host: this}
                } else {
                    D = E.name
                }
                if (H[D]) {
    ...
    A.seek = function (D) {
        var C = D.hiddenDeps || [];
        if (!D.hiddenDeps) {
            var B = D.toString(), i = null;
            C = D.hiddenDeps = [];
            while (i = g.exec(B)) {
                C.push(i[0].slice(10, -2))
            }
        }
        return C.slice()
    };
    A.tidy = function (B, i) {
        t.call(B.slice(), function (E, D) {
            var C = this[i.url + ":" + A.realname(E)];
            if (typeof C === "string") {
                B[D] = C
            }
        }, e.mods)
    };
    A.namesuffix = function (i) {
        return i.replace(/(.+?)(_src.*)?(\.\w+)$/, function (C, B, E, D) {
            return B + (E && "_combo" || "_pack") + D
        })
    };
    A.filesuffix = function (i) {
        return d.test(i) ? i : i + ".js"
    };
    A.realname = function (i) {
        var B = e.aliases;
        if (B) {
            i = i.replace(n, function (E, D) {
                var C = B[D];
                if (!C || i.indexOf(C) === 0) {
                    return D + "/"
                }
    ...
    A.basename = function (i, B) {
        var C = j.exec(i);
        if (C && B) {
            i = (B.url || "").replace(/[^\/]+$/, "") + C[0]
        }
        return A.resolvename(i)
    };
    A.resolvename = function (i) {
        i = i.replace(r, "$1");
        while (c.test(i)) {
            i = i.replace(c, "/").replace(/(^|[^:])\/\/+/g, "$1/")
        }
        return i
    };
    var z = {};
...
/* @source ../lib/domready.min.js */
;

define("mo/domready", [
    "finish"
], function (i) {
    var d, l = this, k = l.document, g = "addEventListener", b = "attachEvent", f = "DOMContentLoaded", c = "onreadystatechange";
    if (k.readyState === "complete") {
        setTimeout(i, 1)
    } else {
        if (k[g]) {
            d = function () {
                k.removeEventListener("READY", d, false);
                i()
            };
            k[g](f, d, false);
            l[g]("load", i, false)
            ...
            if (k[b]) {
                ...
                var j = false;
                try {
                    j = l.frameElement == null
                } catch (h) {
                }
                if (k.documentElement.doScroll && j) {
                    var a = function () {
                        try {
...
/* autogeneration */
define("../lib/domready.min.js", [], function () {
});
...
/* @source ../lib/bootstrap3.1.1/js/bootstrap.min.js */
;
...
(function () {
    ...
    define('bootstrap3', ['jquery'], '../lib/bootstrap3.1.1/js/bootstrap.min.js');

    define('app', ['jquery', 'bootstrap3', '../lib/domready.min.js'], function ($) {
        var app;
        app = function () {
            var MIN_WIDTH;
            MIN_WIDTH = 990;
            $(window).resize(function () {
                if ($(window).width() <= MIN_WIDTH) {
                    $('#nav_hello a').addClass('tablet-layout');
                    $('#qq').css('top', '634px').css('margin-right', '10px');
                } else {
                    $('#nav_hello a').removeClass('tablet-layout');
                    $('#qq').css('top', '460px').css('margin-right', '40px');
                }
                return $('div.main-row:nth-child(odd) div.row').each(function (index, html) {
                    var text;
                    text = $(html).children('.text').html();
                    if (text == null) {
                        return;
                    }
                    if ($(this).width() <= MIN_WIDTH) {
                        $('html .navbar-static-top:not(navbar-right) .navbar-nav li');
                        return $('html .navbar-static-top #logo').css('margin-right', '20px');
                    } else {
                        $('html .navbar-static-top:not(navbar-right) .navbar-nav li');
                        return $('html .navbar-static-top #logo').css('margin-right', '60px');
                    }
                });
            });
            if ($(window).width() <= MIN_WIDTH) {

...
var ContextMenu = function () {
    ...
    this.createSVGRootElement = function(){
        if($("#layout_drawing").length > 0) {
            ...
            image = draw.image('/resources/img/project_cad.jpg').attr({id:'project-img'}).loaded(function(loader) {
        ...
        if(("#sensor_layout_drawing").length>0){
            ...
            sensorImg = sensor_group.image('/resources/img/cad.jpg').attr({id:'sensor_img'}).loaded(function(loader) {
    ...
    this._getPosition = function(evt,nodetype) {
        ...
        if(id=='sensor_layout_drawing'){
            ...
            center.y = mouseY + $(window).scrollTop()-80;
            center.x  = mouseX + $(window).scrollLeft();
        }else{
                var svgRoot =document.getElementById("station_layout_drawing");
                var ctm, foo, pos;
                foo = {
                    x: null,
                    y: null
                ...
                console.log("center : " + draw.viewbox());
                var top=$(evt.target).offset().top ,
                left= $(evt.target).offset().left,
                parentTop= $(evt.target).parent().offset().top,
                parentLeft=$(evt.target).parent().offset().left;
                console.log("top ： "+ top + " left: "+left + " parentTop: "+parentTop + " parentLeft : " +parentLeft + " top after :  "+ (top-parentTop) + " left after: "+(left-parentLeft));
                console.log(pos.x + "=========pos===========" +pos.y);
                console.log("draw.x() ： " + image.x() + " draw.y() : "+ image.y()  + " draw.cx() ：  " + image.cx()  + "  draw.cy() ： " +image.cy()  );
                console.log( draw.viewbox().y + "==========viewbox offset==========" +draw.viewbox().x);
                console.log( draw.viewbox().height + "========== viewbox height-width==========" +draw.viewbox().width );
                center.y = draw.viewbox().y + draw.viewbox().width/2 ;
                center.x =draw.viewbox().x +  draw.viewbox().height/2 ;

            //offset_top =  $(evt.target).offset().top;
            //offset_left =  $(evt.target).offset().left;
            //var mTop=0,mLeft=0;
        ...
        console.log("offset_left: "+ center.x + " ; offset_top: " + center.y);
    }
                ...
                onItem: function (context, evt) {
                    ...
                    switch (id) {
                        case "new_station":
                            ...
                            //arr.push(num);
                            console.log("***offset_left: "+ center.x + " ; offset_top: " + center.y);
                            svgChildNode = draw.circle(mWidth).center(center.x, center.y ).draggable().fill(mColor ).attr({
                                id     : 'newstation',
                                type   : 'station',
                                num    : num,
                                cursor : 'move',
                        ...
                        case "new_data":
                            var param = {
                                width: mWidth * 2,
                                height: mHeight / 2,
                                offset_left: center.x,
                                offset_top: center.y + 40,
                                color: '#cccccc',
                                id: 'newdataaxial',
                                datatype: 'data_axial',
                                text: '轴向力'
